...
//        String tmp = desiredLang.toLowerCase();
        for (Map.Entry<String, String> enter : LANGUAGE_MAP.entrySet()) {
            if (enter.getValue().equals(desiredLang)) {
                return enter.getKey();
            }
        }
    ...
    public List<String> languages() {
        return Arrays.stream(LANGUAGE_MAP.values().toArray(new String[0])).sorted().collect(Collectors.toList());
    }
...
//            }
     ...
     * @throws
     */
    private String postHttp(String url, List<NameValuePair> nvps) throws Exception {
